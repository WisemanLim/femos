...
CSCBArray CSCBar::m_arrBars; // static member

IMPLEMENT_DYNAMIC(CSCBar, CControlBar);

CSCBar::CSCBar()
{
    m_szMin = CSize(100, 32);
...
BEGIN_MESSAGE_MAP(CSCBar, CControlBar)
    //{{AFX_MSG_MAP(CSCBar)
    ON_WM_CREATE()
    ON_WM_PAINT()
    ON_WM_NCPAINT()
...
BOOL CSCBar::Create(LPCTSTR lpszWindowName, CWnd* pParentWnd,
...
/////////////////////////////////////////////////////////////////////////
// CSCBar message handlers

int CSCBar::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
const BOOL CSCBar::IsFloating() const
{
    return !IsHorzDocked() && !IsVertDocked();
}

const BOOL CSCBar::IsHorzDocked() const
{
    return (m_nDockBarID == AFX_IDW_DOCKBAR_TOP ||
        m_nDockBarID == AFX_IDW_DOCKBAR_BOTTOM);
}

const BOOL CSCBar::IsVertDocked() const
{
    return (m_nDockBarID == AFX_IDW_DOCKBAR_LEFT ||
        m_nDockBarID == AFX_IDW_DOCKBAR_RIGHT);
}

const BOOL CSCBar::IsSideTracking() const
{
...
CSize CSCBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
...
CSize CSCBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
...
void CSCBar::OnWindowPosChanging(WINDOWPOS FAR* lpwndpos)
{
...
// Mouse Handling
//
void CSCBar::OnLButtonDown(UINT nFlags, CPoint point) 
{
	RECT rt;
	GetClientRect(&rt);
	if( point.x > rt.left && point.x < rt.right && point.y > rt.top && point.y < rt.bottom ) CWnd::OnLButtonDown(nFlags, point);
	else if (m_pDockBar != NULL )
	{
		// start the drag
...
void CSCBar::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
    if (m_pDockBar != NULL)
    {
        // toggle docking
...
void CSCBar::OnNcLButtonDown(UINT nHitTest, CPoint point) 
{
    if (IsFloating())
    {
...
void CSCBar::OnNcLButtonUp(UINT nHitTest, CPoint point) 
{
    if (nHitTest == HTCLOSE)
...
void CSCBar::OnLButtonUp(UINT nFlags, CPoint point) 
{
    if (m_bTracking)
        StopTracking();
...
void CSCBar::OnRButtonDown(UINT nFlags, CPoint point) 
{
    if (m_bTracking)
        StopTracking();
...
void CSCBar::OnMouseMove(UINT nFlags, CPoint point) 
{
    if (m_bTracking)
...
void CSCBar::OnCaptureChanged(CWnd *pWnd) 
{
...
void CSCBar::OnNcCalcSize(BOOL bCalcValidRects,
...
void CSCBar::NcPaintGripper(CDC* pDC, CRect rcClient)
{
...
UINT CSCBar::OnNcHitTest(CPoint point)
{
    if (IsFloating())
...
void CSCBar::OnSettingChange(UINT uFlags, LPCTSTR lpszSection) 
{
...
/////////////////////////////////////////////////////////////////////////
// CSCBar implementation helpers

void CSCBar::StartTracking(UINT nHitTest)
{
    SetCapture();
...
void CSCBar::OnTrackUpdateSize(CPoint& point)
{
    ASSERT(!IsFloating());
...
void CSCBar::OnTrackInvertTracker()
{
    ASSERT(m_bTracking);
...
BOOL CSCBar::GetEdgeRect(CRect rcWnd, UINT nHitTest,
                                    CRect& rcEdge)
{
    rcEdge = rcWnd;
    if (m_dwSCBStyle & SCBS_SHOWEDGES)
...
UINT CSCBar::GetEdgeHTCode(int nEdge)
{
...
void CSCBar::GetRowInfo(int& nFirst, int& nLast, int& nThis)
{
...
void CSCBar::GetRowSizingBars(CSCBArray& arrSCBars)
{
    arrSCBars.RemoveAll();
    ...
    for (int i = nFirst; i <= nLast; i++)
    {
        ...
        if (FindSizingBar(pBar) >= 0)
            arrSCBars.Add((CSCBar*)pBar);
    }
}

const int CSCBar::FindSizingBar(CControlBar* pBar) const
{
...
BOOL CSCBar::NegociateSpace(int nLengthAvail, BOOL bHorz)
{
...
void CSCBar::AlignControlBars()
{
    ...
    if (bNeedRecalc) m_pDockSite->DelayRecalcLayout();
}

void CSCBar::OnUpdateCmdUI(CFrameWnd* pTarget, BOOL bDisableIfNoHndler)
{
...
void CSCBar::LoadState(LPCTSTR lpszProfileName)
{
    ASSERT_VALID(this);
...
void CSCBar::SaveState(LPCTSTR lpszProfileName)
{
...
void CSCBar::GlobalLoadState(LPCTSTR lpszProfileName)
{
    for (int i = 0; i < m_arrBars.GetSize(); i++)
        ((CSCBar*) m_arrBars[i])->LoadState(lpszProfileName);
}

void CSCBar::GlobalSaveState(LPCTSTR lpszProfileName)
{
    for (int i = 0; i < m_arrBars.GetSize(); i++)
        ((CSCBar*) m_arrBars[i])->SaveState(lpszProfileName);
}
