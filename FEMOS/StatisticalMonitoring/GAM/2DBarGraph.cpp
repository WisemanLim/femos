		...
		if ( m_AnimationType == AT_BAR_DRAW_ALL )
		{
			...
			if ( ( m_SegmentsNumber > 0 ) && ( m_SeriesNumber > 0 ) )
			{
				...
				while ( curr_seg != NULL )
				{
					...
					::SetBkMode( hDC, TRANSPARENT );
					RECT rt;
					SetRect( &rt, ser_start.x-int(m_SeriesNumber*ser_step), startY+int(m_Size.cy*0.01), ser_start.x + int((seg_width + m_SeriesNumber*ser_step*2)), startY+int(m_Size.cy*0.01)+30 );
					::DrawText( hDC, curr_seg->text, curr_seg->text.GetLength(), &rt, DT_CENTER | DT_NOCLIP | DT_END_ELLIPSIS );
					//::TextOut( hDC, ser_start.x + int((seg_width + m_SeriesNumber*ser_step)/2), startY+int(m_Size.cy*0.01)   , curr_seg->text.Left(6), 6 );
					::SetTextAlign( hDC, TA_LEFT );
					::SetBkMode( hDC, OPAQUE );
		...
		else if ( m_AnimationType == AT_BAR_DRAW_SERIES )
		{
			...
			if ( ( m_SegmentsNumber > 0 ) && ( m_SeriesNumber > 0 ) )
			{
				...
				while ( curr_seg != NULL )
				{
					// Draw segment text
					RECT rt;
					SetRect( &rt, seg_font_pos.x-int(m_SeriesNumber*ser_step), startY+int(m_Size.cy*0.01), seg_font_pos.x + int((seg_width + m_SeriesNumber*ser_step*2)), startY+int(m_Size.cy*0.01)+30 );
					::DrawText( hDC, curr_seg->text, curr_seg->text.GetLength(), &rt, DT_CENTER | DT_NOCLIP | DT_END_ELLIPSIS );
				...
				while ( curr_seg != NULL )
				{
					...
					if ( m_BlendSegmentsCompleted < curr_seg->index )
					{
						...
						while ( c_seg->index < curr_seg->index )
						{
							...
							RECT rt;
							SetRect( &rt, ser_start.x-int(m_SeriesNumber*ser_step), startY+int(m_Size.cy*0.01), ser_start.x + int((seg_width + m_SeriesNumber*ser_step*2)), startY+int(m_Size.cy*0.01)+30 );
							::DrawText( hDC, c_seg->text, c_seg->text.GetLength(), &rt, DT_CENTER | DT_NOCLIP | DT_END_ELLIPSIS );
