...
BOOL CGridCtrl::PasteTextToGrid(CCellID cell, COleDataObject* pDataObject)
{
	...
	SendMessageToParent(-1, -1, GVN_BEGINPASTE);
	...
	SendMessageToParent(-1, -1, GVN_ENDPASTE);
...
void CGridCtrl::OnBeginDrag()
{
    ...
    if (pSource)
    {
        ...
        if (dropEffect & DROPEFFECT_MOVE)
            //CutSelectedText();//Edited by 상하 to 드래그하면 카피되게..
...
void CGridCtrl::AutoSize(BOOL bIgnoreColumnHeader)
{
	...
	int nCol, nRow;
	int iStartRow = 0;
	int iDefaultSizeX, iDefaultSizeY;
	CDC* pDC = GetDC();
	if (!pDC) return;
	GetClientRect(&rt);
	iDefaultSizeX = rt.right/nNumColumns-2;
	iDefaultSizeY = rt.bottom/nNumRows;
	if( iDefaultSizeX > 100 ) iDefaultSizeX = 100;
	if( iDefaultSizeY > 18 ) iDefaultSizeY = 18;
	if( bIgnoreColumnHeader ) iStartRow = GetFixedRowCount();
			...
			for (nRow = iStartRow; nRow < nNumRows; nRow++)
			{
				if( GetRowHeight( nRow) > 0 )
				{
					...
					if (size.cx >(int) m_arColWidths[nCol])
						if( size.cx < iDefaultSizeX ) m_arColWidths[nCol] = iDefaultSizeX;
						else m_arColWidths[nCol] = size.cx;
					if (size.cy >(int) m_arRowHeights[nRow])
						if( size.cy < iDefaultSizeY ) m_arRowHeights[nRow] = iDefaultSizeY;
						else m_arRowHeights[nRow] = size.cy;
				}
			}
		}
	}
