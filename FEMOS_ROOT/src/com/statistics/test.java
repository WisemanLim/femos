...
public class test {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// REngine is the backend-agnostic API -- using eng instead of c
			// makes sure that we don't use Rserve extensions inadvertently
			REngine eng = (REngine) c;
			
			System.out.println(">>"
					+ c.eval("mean(abs(rnorm(100)))").asString() + "<<");
			
			{
				/*
				 * drv <- JDBC("oracle.jdbc.OracleDriver", classPath="/oracle/product/102/db/oc4j/jdbc/ojdbc14.jar", " ")
				 * con <- dbConnect(drv, "jdbc:oracle:thin:@hbsys98.vps.phps.kr:1521:FEMOS", "hbsys", "hbsys2012")
				 * dbGetQuery(con, "select * from tab")
				 * dbDisconnect(con)
				 */
				System.out.println("1");
				c.voidEval("library(RJDBC)");
				System.out.println("2");
				c.assign("drv", "JDBC(\"oracle.jdbc.OracleDriver\", classPath=\"/oracle/product/102/db/oc4j/jdbc/ojdbc14.jar\", \" \")");
				System.out.println("3");
				c.assign("con", "dbConnect(drv, \"jdbc:oracle:thin:@hbsys98.vps.phps.kr:1521:FEMOS\", \"hbsys\", \"hbsys2012\")");
				System.out.println("4");
				c.voidEval("dbGetQuery(con, \"select * from tab\")");
/*				String l[] = c.eval("dbGetQuery(con, \"select * from tab\")").asStrings();
				System.out.println("5");
				for ( int i=0; i<l.length; i++ ) {
					System.out.println("List << [" + i + "] >> -- " + l[i].toString());
				}*/
				System.out.println("6");
				c.voidEval("dbDisconnect(con)");
...
/*			System.out.println(">>"
...

			{
				...
				RList l = c
						.eval("{d=data.frame(\"huhu\",c(11:20)); lapply(d,as.character)}")
						.asList();
				int cols = l.size();
			...
			{
				System.out
						.println("* Test NA/NaN support in double vectors...");
				...
				if (!nas.equals("[1] 1.0 0.5  NA NaN 3.5"))
					throw new TestException(
							"NA/NaN assign+retrieve test failed");
				REXP rx = c.eval("c(2.2, NA_real_, NaN)");
				if (rx == null || !rx.isNumeric() || rx.isInteger())
					throw new TestException(
							"NA/NaN pull test failed (invalid result)");
				boolean nal[] = rx.isNA();
				if (nal.length != 3 || nal[0] || !nal[1] || nal[2])
					throw new TestException("NS/NAN pull test: NA pull failed");
				x = rx.asDoubles();
				if (x.length != 3 || !Double.isNaN(x[2])
						|| REXPDouble.isNA(x[2]) || !REXPDouble.isNA(x[1]))
					throw new TestException(
							"NS/NAN pull test: NA/NaN pull failed");
				System.out.println("PASSED");
			}
			...
			}
			{
				...
				if (!x.isLogical() || x.length() != 3)
					throw new TestException(
							"boolean array assign+retrieve test failed");
				boolean q[] = ((REXPLogical) x).isTRUE();
				if (q[0] != true || q[1] != false || q[2] != true)
					throw new TestException(
							"boolean array assign+retrieve test failed (value mismatch)");
				...
				if (q1[0] != true || q1[1] != false || q1[2] != false
					...
					throw new TestException(
							"boolean array NA test failed (value mismatch)");
			}

			{ // regression: object bit was not set for Java-side generated
				// objects before 0.5-3
				System.out
						.println("* Testing functionality of assembled S3 objects ...");
				// we have already assigned the data.frame in previous test, so
				// we jsut re-use it
				REXP x = c.parseAndEval("z[2,2]");
				System.out.println("  z[2,2] = " + x);
				if (x == null || x.length() != 1 || x.asDouble() != 1.2)
					throw new TestException(
							"S3 object bit regression test failed");
				System.out.println("PASSED");
			}

			{ // this test does a pull and push of a data frame. It will fail
				// when the S3 test above failed.
				System.out
						.println("* Testing pass-though capability for data.frames ...");
				REXP df = c.parseAndEval("{data(iris); iris}");
				c.assign("df", df);
				REXP x = c.eval("identical(df, iris)");
				System.out.println("  identical(df, iris) = " + x);
				if (x == null || !x.isLogical() || x.length() != 1
						|| !((REXPLogical) x).isTrue()[0])
					throw new TestException(
							"Pass-through test for a data.frame failed");
				System.out.println("PASSED");
			}
			...
			{ // factors
				...
				REXP f = c
						.parseAndEval("factor(paste('F',as.integer(runif(20)*5),sep=''))");
				...
				if (!f.isFactor() || f.asFactor() == null)
					throw new TestException(
							"single factor test failed (not a factor)");
				if (!f.asFactor().at(0).equals("foo"))
					throw new TestException(
							"single factor test failed (wrong value)");
				System.out
						.println("  test factors with null elements contents:");
				c.assign("f", new REXPFactor(new RFactor(new String[] { "foo",
						"bar", "foo", "foo", null, "bar" })));
				f = c.parseAndEval("f");
				if (!f.isFactor() || f.asFactor() == null)
					throw new TestException(
							"factor assign-eval test failed (not a factor)");
			...
			}
			{
				...
				if (c.eval("identical(m ,m2)").asInteger() != 1)
					throw new TestException(
							"matrix test failed: createDoubleMatrix result is not the same as direct assign");
				System.out.println("matrix: cross-product");
				double[][] mr = c.parseAndEval("crossprod(m,m)")
						.asDoubleMatrix();
				System.out.println("PASSED");
			}

			{
				...
				System.out.println("  serialized ls is " + b.length
						+ " bytes long");
				c.assign("r", new REXPRaw(b));
				String[] s = c.eval("unserialize(r)()").asStrings();
				System.out.println("  we have " + s.length
						+ " items in the workspace");
				System.out.println("PASSED");
			}

			{ //
			 // NAs in character vectors are mapped to null references in
			 // String[] and vice versa. Only Rserve 0.6-2 and later support NAa
			 // in character vectors.
			 // repression test: assigning empty string '' has failed in
			 // previous versions of RserveEngine
				System.out
						.println("* Test handling of NAs in character vectors ('foo', '', NA, 'NA')");
				...
				if (nas.length != 4 || nas[0] != REXPLogical.FALSE
						|| nas[1] != REXPLogical.FALSE
						|| nas[2] != REXPLogical.TRUE
						|| nas[3] != REXPLogical.FALSE)
					throw new TestException(
							"assigning null Strings as NAs and '' has failed");
				System.out.println(" - OK");
				System.out.print("  pull String[] with NAs: ");
				String s[] = eng.parseAndEval("c('foo', '', NA, 'NA')")
						.asStrings();
				for (int i = 0; i < s.length; i++)
					System.out.print("'" + s[i] + "' ");
				if (s.length != 4 || s[0] == null || s[1] == null
						|| !s[1].equals("") || s[2] != null || s[3] == null)
					throw new TestException(
							"pulling Strings containin NAs and '' has failed");
				System.out.println(" - OK");
				System.out.print("  compare pushed and constructed strings: ");
				if (eng.parseAndEval("identical(s, c('foo', '', NA, 'NA'))")
						.asInteger() != REXPLogical.TRUE)
					throw new TestException(
							"comparing Strings with NAs and '' has failed");
				System.out.println(" - OK");
			...
			{ // string encoding test (will work with Rserve 0.5-3 and higher
				// only)
				...
				if (x == null || !x.isInteger() || x.asInteger() != 4)
					throw new TestException(
							"UTF-8 encoding string length test failed");
				// we cannot really test any other encoding ..
				System.out.println("PASSED");
			}

			{ // test control commands (works only when enabled and in Rserve
				// 0.6-0 and higher only) - must be the last test since it
				// closes the connection and shuts down the server
				System.out
						.println("* Test control commands (this will fail if control commands are disabled) ...");
				System.out.println("  server eval");
				String key = "rn" + Math.random(); // generate a random number
				...
				if (x == null || !x.isString() || x.length() != 1
					...
					throw new TestException(
							"control eval test failed - assignment was not persistent");
				c.serverEval("rm(xXx)"); // remove the test variable to not
											// pollute the global workspace
