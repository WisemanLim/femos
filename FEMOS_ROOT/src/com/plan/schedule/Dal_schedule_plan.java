...
public class Dal_schedule_plan 
{
	...
	public void getSchedule_list_date(String festival_cd,String festival_hall_cd,String yyyymmdd)
	{
		DBConnection dbo = new DBConnection();
		Statement stmt = null;
		ResultSet rs = null; 
		...
		try
		{
			...
			sql = sql + "WHERE A.PROGRAM_CD = B.PROGRAM_CD(+) ";
			sql = sql + "AND A.FESTIVAL_CD = '"+festival_cd+"' AND FESTIVAL_HALL_CD = '"+festival_hall_cd+"'";
			sql = sql + "AND '"+yyyymmdd+"' BETWEEN (A.ST_YEAR||A.ST_MONTH||A.ST_DAY) AND (A.ED_YEAR||A.ED_MONTH||A.ED_DAY) ";
			sql = sql + "ORDER BY A.ST_YEAR||A.ST_MONTH||A.ST_DAY ASC";
